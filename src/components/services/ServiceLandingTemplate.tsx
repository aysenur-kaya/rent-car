...
import { ServiceCarFleet } from "@/components/services/ServiceCarFleet";
...
export function ServiceLandingTemplate({ service }: ServiceLandingTemplateProps) {
  ...
  return (
    <>
      ...
      <section className={section}>
        <Container>
          <div className="mx-auto max-w-3xl">
            <h2 className="text-h2 text-foreground">Hizmet Detayı</h2>
      ...
      <section className={cn(section, "bg-background pt-0")} aria-labelledby={`${service.slug}-advantages`}>
        <Container>
          <div className="mb-8 max-w-2xl">
            <span className="badge-light mb-3">{service.badge}</span>
            <h2 id={`${service.slug}-advantages`} className="text-h2 text-foreground">
              {service.name} Avantajları
            </h2>
          </div>
      ...
      <ServiceCarFleet
        categoryFilter={service.carCategoryFilter}
        serviceName={service.name}
      />

      <WhyUs />

      <section className={cn(section, "bg-background")} aria-labelledby={`${service.slug}-faq`}>
        <Container>
          <div className="mx-auto max-w-3xl">
            <div className="mb-8 text-center">
              <span className="badge-light mb-3">SSS</span>
              <h2 id={`${service.slug}-faq`} className="text-h2 text-foreground">
                {service.name} Hakkında Sık Sorulan Sorular
              </h2>
            </div>
